...
void ajouterEtudiant(int matricules[max], char noms[max][50], char prenoms[max][50], int* nbEtudiants) {
    ...
    do{
        printf("Matricule : ");
        scanf("%ld",&matricule);
    }while(matricule>9999||matricule<0);
...
void modifierEtudiant(int matricules[max], char noms[max][50], char prenoms[max][50], int nbEtudiants) {
    ...
    e=existe(matricules,nbEtudiants,matricule);
...
int existe(int matricules[max], int nbEtudiants, int matricule) {
    int i;
    for (i=0;i<nbEtudiants; i++) {
        if (matricule == matricules[i]) {
            return i;
...
void Ajouternote(long int matricules[max], float notesetudiants[max][5],int nbEtudiants) {
    ...
    printf("\n La note generale et :  %.2f  ",notesetudiants[Existe][6]);
...
void bulletinS(long int matricules[max],float notesetudiants[max][5] ,float bulletin[max], int nbEtudiants,char noms[max][50], char prenoms[max][50]) {
...
int main() {
int i,j;
int choixPrincipal,choixGestion;
float notesetudiants[max+2][6];
long int matricules[max];
char noms[max][50];
char prenoms[max][50];
int nbEtudiants = 0;
int choixmodifieretudiant;
